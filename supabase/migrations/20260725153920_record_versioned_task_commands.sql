-- PR-8a: first record-level domain command.
-- Task updates use their own optimistic-lock version. The organization state
-- version is still advanced as a safety fence for legacy whole-state writes,
-- but it is never used to reject updates of different task records.
...
create policy "editors insert task command audit" on public.audit_events
...
begin
  ...
  if p_organization_id is null or not exists (
    ...
    raise exception 'Brak uprawnień do zapisu zadania' using errcode = '42501';
  end if;
  if p_task_id is null or length(trim(p_task_id)) < 1 or length(p_task_id) > 128 then
    raise exception 'Nieprawidłowy identyfikator zadania' using errcode = '22023';
  end if;
  if p_expected_record_version is null or p_expected_record_version < 1 then
    raise exception 'Nieprawidłowa wersja zadania' using errcode = '22023';
  end if;
  ...
  if p_task is null
    ...
    or length(trim(coalesce(p_task ->> 'bookingId', ''))) < 1
    or length(trim(coalesce(p_task ->> 'owner', ''))) < 1
    or length(trim(coalesce(p_task ->> 'title', ''))) < 1
    or coalesce(p_task ->> 'type', '') not in (
      'Dane', 'Rezerwacja', 'Płatność', 'Przed przyjazdem', 'Sprzątanie',
      'Content', 'Opinia', 'Follow-up', 'Naprawa', 'Inne'
    )
    or coalesce(p_task ->> 'priority', '') not in ('Wysoki', 'Średni', 'Niski')
    or coalesce(p_task ->> 'status', '') not in (
      'Do zrobienia', 'W toku', 'Zrobione', 'Zablokowane', 'Nie dotyczy'
    )
  then
    raise exception 'Zadanie narusza reguły domenowe' using errcode = '22023';
  end if;

  if p_task ? 'dueDate' and p_task ->> 'dueDate' is not null then
    perform (p_task ->> 'dueDate')::date;
  end if;

  committed_task := p_task || jsonb_build_object(
    'version', p_expected_record_version + 1,
    'updatedAt', committed_at
  );

  update public.operational_records
  set payload = committed_task,
...
revoke all on function public.update_operational_task(uuid, text, bigint, jsonb, text, timestamptz, text)
  from public, anon;
grant execute on function public.update_operational_task(uuid, text, bigint, jsonb, text, timestamptz, text)
  to authenticated;
