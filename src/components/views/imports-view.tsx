...
export function ImportsView() {
  const { data, replaceWithImportedBookings, updateConnection, importAdSpend, exportSnapshot, exportPricingAnalysis } = useAppStore();
  ...
  async function selectAdSpendFile(file?: File) {
    if (!file) return;
    try {
      const parsed = parseAdSpendCsv(await file.text(), file.name);
      if (!parsed.records.length) {
        setMessage(parsed.errors.join(" · ") || "Nie rozpoznano wydatków reklamowych.");
        return;
      }
      importAdSpend(parsed.records);
      setMessage(`Zaimportowano ${parsed.records.length} pozycji reklamowych${parsed.errors.length ? `; pominięto ${parsed.errors.length} błędnych wierszy` : ""}. Kampanie są pokazane jako korelacja, nie przyczyna.`);
    } catch {
      setMessage("Nie udało się odczytać pliku wydatków reklamowych.");
    }
  }
  ...
  return <div className="grid gap-5">
    ...
    <div className="grid gap-5 xl:grid-cols-[1fr_360px]">
      ...
      <aside className="grid content-start gap-5"><Card className="p-5"><p className="text-[10px] font-black uppercase tracking-[.16em] text-[#7d8b4d]">Eksport Stawy OS</p><h2 className="font-display text-xl font-semibold">Linki dostępności</h2><p className="mt-2 text-xs leading-5 text-[#68756f]">Każdy domek ma prywatny adres. Skopiuj właściwy link do ustawień importu kalendarza w Booking lub Airbnb.</p><div className="mt-4 grid gap-2">{data.units.map((unit) => feedUrls[unit.id] ? <a className="flex items-center justify-between rounded-xl border border-[#d8d0c2] bg-white p-3 text-xs font-black" href={feedUrls[unit.id]} key={unit.id} rel="noreferrer" target="_blank">{unit.name}<Icon className="size-4" name="download"/></a> : <span className="rounded-xl border border-[#d8d0c2] bg-[#f5f2eb] p-3 text-xs font-black text-[#77827c]" key={unit.id}>{unit.name} · generuję link…</span>)}</div></Card><Card className="p-5"><p className="text-[10px] font-black uppercase tracking-[.16em] text-[#7d8b4d]">Kopia bezpieczeństwa</p><h2 className="font-display text-xl font-semibold">Szyfrowany eksport</h2><p className="mt-2 text-xs leading-5 text-[#68756f]">Pełna kopia jest szyfrowana hasłem jeszcze w przeglądarce. Zapisz ją przed dużym importem.</p><Button className="mt-4 w-full" variant="secondary" onClick={() => setShowBackup(true)}><Icon className="size-4" name="download"/>Pobierz zaszyfrowany backup</Button></Card><Card className="p-5"><p className="text-[10px] font-black uppercase tracking-[.16em] text-[#7d8b4d]">Analiza cen</p><h2 className="font-display text-xl font-semibold">Dane dla AI bez danych gości</h2><p className="mt-2 text-xs leading-5 text-[#68756f]">Eksport obejmuje terminy, obłożenie, ceny, kanały, prowizje i koszty. Nie zawiera nazwisk, telefonów, e-maili ani wiadomości.</p><Button className="mt-4 w-full" variant="secondary" onClick={exportPricingAnalysis}><Icon className="size-4" name="download"/>Pobierz zestaw cenowy</Button></Card></aside>
    </div>

    <Card className="overflow-hidden border-[#cedbb9] bg-[#fbfcf5]"><div className="grid gap-5 p-5 lg:grid-cols-[1fr_320px] sm:p-6"><div><p className="text-[10px] font-black uppercase tracking-[.16em] text-[#66813d]">Import jednorazowy i historyczny</p><h2 className="mt-1 font-display text-2xl font-semibold">Mobile Calendar + rozliczenia OTA</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[#5d6b61]">Najpierw wybierz najnowszy eksport Mobile Calendar. Opcjonalne rozliczenia Airbnb i Booking wzbogacą te same rezerwacje o faktyczną cenę brutto, prowizje, opłaty płatnicze i wypłatę netto.</p><FilePicker fileName={fileName} label="Mobile Calendar · wymagany" note="Pełny eksport rezerwacji CSV" onSelect={selectFile}/><div className="mt-3 grid gap-3 sm:grid-cols-2"><FilePicker compact fileName={airbnbFileName} label="Airbnb · rozliczenia" note="Historia transakcji CSV" onSelect={(file) => selectFinancialFile("Airbnb", file)}/><FilePicker compact fileName={bookingFileName} label="Booking · wypłaty" note="Payout CSV, nie pusty wydruk PDF" onSelect={(file) => selectFinancialFile("Booking", file)}/></div><details className="mt-3"><summary className="cursor-pointer text-xs font-black text-[#4a6c50]">Wklej dane Mobile Calendar ręcznie zamiast pliku</summary><textarea aria-label="Dane z Mobile Calendar" className="mt-3 min-h-36 w-full rounded-2xl border border-[#cfd8c2] bg-white px-4 py-3 font-mono text-xs leading-5 outline-none focus:border-[#759655]" value={rawImport} onChange={(event) => { setRawImport(event.target.value); setFileName(""); setPreview(null); }} placeholder={"Wklej zawartość eksportu CSV…"}/></details></div><div className="rounded-2xl bg-[#e8f0dd] p-5"><span className="grid size-10 place-items-center rounded-xl bg-white text-[#477346]"><Icon className="size-5" name="refresh"/></span><p className="mt-4 text-sm font-black">Najpierw kontrola, potem zapis</p><p className="mt-2 text-xs leading-5 text-[#60725e]">Podgląd pokaże nowe i wzbogacane pobyty, dopasowania finansowe oraz rekordy tylko z OTA. Historyczne odbyte pobyty zostaną oznaczone jako zamknięte i rozliczone.</p><Button className="mt-5 w-full" disabled={busy || !rawImport.trim()} onClick={() => void previewImport()}>1. Sprawdź dane</Button><Button className="mt-2 w-full" disabled={busy || !preview?.rows.length} variant="secondary" onClick={() => void commitImport()}>2. Scal {preview?.rows.length ?? 0} rekordów</Button></div></div>{preview ? <PreviewTable data={data} preview={preview}/> : null}</Card>

    <Card className="overflow-hidden"><div className="grid gap-5 p-5 sm:grid-cols-[1fr_auto] sm:items-center"><div><p className="text-[10px] font-black uppercase tracking-[.16em] text-[#7d8b4d]">PR-11d · najpierw CSV</p><h2 className="font-display text-2xl font-semibold">Wydatki reklamowe</h2><p className="mt-2 text-sm leading-6 text-[#65736c]">Kolumny: data, kanał, kampania, koszt, waluta, opcjonalnie wynik i kod/UTM. Import nie uruchamia reklamy i nie nazywa korelacji przyczynowością.</p></div><label className="inline-flex min-h-11 cursor-pointer items-center justify-center gap-2 rounded-xl bg-[#174d3b] px-4 text-sm font-black text-white"><Icon className="size-4" name="upload"/>Wgraj CSV reklam<input accept=".csv,text/csv" className="sr-only" type="file" onChange={(event)=>void selectAdSpendFile(event.target.files?.[0])}/></label></div></Card>
...
function FilePicker({
  ...
  return <label className={`${compact ? "" : "mt-4"} flex cursor-pointer items-center gap-3 rounded-2xl border border-dashed border-[#9dad7b] bg-white p-4 transition hover:border-[#50734a] hover:bg-[#f7faef]`}><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-[#e6efdc] text-[#3f6c48]"><Icon className="size-5" name="upload"/></span><span className="min-w-0"><span className="block truncate text-sm font-black">{fileName || label}</span><span className="block truncate text-xs text-[#69766f]">{note}</span></span><input accept=".csv,text/csv" className="sr-only" type="file" onChange={(event) => void onSelect(event.target.files?.[0])}/></label>;
}

function PreviewTable({ data, preview }: { data: ReturnType<typeof useAppStore>["data"]; preview: ImportPreview }) {
  const financialByBooking = new Map(
    preview.imports.filter((item) => item.matchedBookingId).map((item) => [item.matchedBookingId!, item]),
  );
  const financial = preview.financialSummary;
  return <div className="border-t border-[#ded7ca] p-4">
    <div className="mb-4 grid gap-2 sm:grid-cols-4">
      <ImportMini label="Wszystkie" value={preview.summary.total}/>
      <ImportMini label="Historia" value={preview.summary.historical}/>
      <ImportMini label="Aktywne" value={preview.summary.active}/>
      <ImportMini label="Do sprawdzenia" value={preview.summary.needsReview} warn={preview.summary.needsReview>0}/>
    </div>
    {financial ? <div className="mb-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
      <ImportMini label="Dopasowane finanse" value={financial.matchedFinancial} note={financial.dateDiscrepancyMatches ? `${financial.dateDiscrepancyMatches} z różnicą terminu` : undefined}/>
      <ImportMini label="Tylko w OTA" value={financial.otaOnlyBookings} warn={financial.otaOnlyBookings>0}/>
      <ImportMini label="Uznane za rozliczone" value={financial.historicalSettled}/>
      <ImportMini label="Finanse do kontroli" value={financial.financialReview} warn={financial.financialReview>0}/>
    </div> : null}
    <div className="overflow-x-auto"><table className="w-full min-w-[1100px] text-left text-sm"><thead className="text-[10px] font-black uppercase tracking-[.13em] text-[#78837d]"><tr><th className="p-3">Gość</th><th>Termin</th><th>Domek</th><th>Stan</th><th className="text-right">Brutto</th><th className="text-right">Opłaty OTA</th><th className="text-right">Wypłata</th><th>Kontrola</th></tr></thead><tbody>{preview.rows.map((booking:Booking)=>{const source=financialByBooking.get(booking.id);return <tr className="border-t" key={booking.id}><td className="p-3"><p className="font-black">{booking.guestLabel}</p><p className="text-xs text-[#6e7973]">{booking.platform}</p></td><td>{formatPolishDate(booking.checkIn)} – {formatPolishDate(booking.checkOut)}<p className="text-xs">{nightsBetween(booking.checkIn,booking.checkOut)} nocy</p></td><td>{unitName(data.units,booking.unitId)}</td><td><Badge tone={data.bookings.some((item)=>item.id===booking.id)?"warn":"good"}>{data.bookings.some((item)=>item.id===booking.id)?"Wzbogacenie":"Nowy"}</Badge></td><td className="text-right font-bold">{booking.grossPrice?.toLocaleString("pl-PL") ?? "—"} {booking.currency ?? ""}</td><td className="text-right">{source?.totalOtaFees?.toLocaleString("pl-PL") ?? "—"}</td><td className="text-right">{source?.payout?.toLocaleString("pl-PL") ?? "—"}</td><td className="max-w-[280px] text-xs">{booking.importWarnings?.length ? <span className="font-bold text-[#9a402b]">{booking.importWarnings.join(" · ")}</span> : source?.matchConfidence==="Do sprawdzenia" ? <span className="font-bold text-[#9a402b]">Rekord tylko w rozliczeniu OTA</span> : <span className="font-bold text-[#3f6e4c]">Gotowe</span>}</td></tr>})}</tbody></table></div>
    <p className="mt-3 text-xs font-bold text-[#586b61]">Suma rezerwacji: {preview.summary.plnTotal.toLocaleString("pl-PL")} PLN{preview.summary.eurTotal ? ` · ${preview.summary.eurTotal.toLocaleString("pl-PL")} EUR` : ""}{financial ? ` · opłaty OTA ${financial.plnCommission.toLocaleString("pl-PL")} + ${financial.plnPaymentProcessing.toLocaleString("pl-PL")} PLN` : ""}</p>
    {preview.errors.length?<div className="mt-3 rounded-xl bg-[#f9dfd7] p-3 text-xs font-bold text-[#963c27]">Pominięte wiersze: {preview.errors.map((item)=>`${item.line}: ${item.message}`).join(" · ")}</div>:null}
  </div>;
}

function ImportMini({label,value,warn=false,note}:{label:string;value:number;warn?:boolean;note?:string}) { return <div className={`rounded-xl border p-3 ${warn?"border-[#ecc5b8] bg-[#f9e7df]":"border-[#dbe1cf] bg-white"}`}><p className="text-[9px] font-black uppercase tracking-[.13em] text-[#7c8780]">{label}</p><p className="font-display text-xl font-semibold">{value}</p>{note?<p className="mt-0.5 text-[10px] font-bold text-[#7a857e]">{note}</p>:null}</div>; }

function ConnectionDialog({connection,units,onClose,onSave}:{connection:SourceConnection;units:ReturnType<typeof useAppStore>["data"]["units"];onClose:()=>void;onSave:(value:SourceConnection)=>void}) { const [form,setForm]=useState(connection); return <Dialog ariaLabel={`Konfiguracja ${connection.platform}`} className="w-full max-w-xl rounded-[22px] bg-[#fffdf8] shadow-2xl" onClose={onClose} overlayClassName="grid place-items-center"><form className="p-6" onSubmit={(event)=>{event.preventDefault();onSave({...form,status:form.importUrl?"Wymaga sprawdzenia":"Do podłączenia",connectionType:"iCal",nextStep:form.importUrl?"Uruchom kontrolę połączenia.":"Wklej prywatny adres eksportu iCal z panelu portalu."});}}><div className="flex items-start justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.16em] text-[#7d8b4d]">iCal</p><h2 className="font-display text-2xl font-semibold">{connection.platform}</h2></div><button aria-label="Zamknij" type="button" onClick={onClose}><Icon className="size-5" name="close"/></button></div><div className="mt-5 grid gap-4"><Field label="Domek"><select data-dialog-initial-focus className={inputClass} required value={form.unitId??""} onChange={(event)=>setForm({...form,unitId:event.target.value})}><option value="">Wybierz domek</option>{units.map((unit)=><option key={unit.id} value={unit.id}>{unit.name}</option>)}</select></Field><Field label="Prywatny adres importu .ics" hint="Adres zawiera sekret. Nie publikuj go i nie wklejaj do wiadomości."><input className={inputClass} type="url" placeholder="https://…/calendar.ics" value={form.importUrl??""} onChange={(event)=>setForm({...form,importUrl:event.target.value})}/></Field><div className="rounded-xl bg-[#f5ead0] p-3 text-xs leading-5 text-[#725a1d]">Synchronizacja iCal nie jest natychmiastowa. Połączenie blokuje dostępność, lecz nie przenosi ceny ani danych gościa.</div></div><div className="mt-6 flex justify-end gap-2"><Button type="button" variant="ghost" onClick={onClose}>Anuluj</Button><Button type="submit">Zapisz konfigurację</Button></div></form></Dialog> }
