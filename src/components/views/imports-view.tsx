...
export function ImportsView() {
  ...
  return <div className="grid gap-5">
    ...
    <Card className="overflow-hidden border-[#cedbb9] bg-[#fbfcf5]"><div className="grid gap-5 p-5 lg:grid-cols-[1fr_320px] sm:p-6"><div><p className="text-[10px] font-black uppercase tracking-[.16em] text-[#66813d]">Import jednorazowy i historyczny</p><h2 className="mt-1 font-display text-2xl font-semibold">Mobile-Calendar: podgląd i scalanie</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[#5d6b61]">Kolumny podstawowe: numer, gość, domek, przyjazd, wyjazd, płatność. Dalej opcjonalnie: kanał, cena, data rezerwacji, dorośli, dzieci, prowizja, wypłata i status. Dla historii wpisz „-” zamiast gościa — analiza cen nie potrzebuje nazwiska.</p><textarea aria-label="Dane z Mobile-Calendar" className="mt-4 min-h-36 w-full rounded-2xl border border-[#cfd8c2] bg-white px-4 py-3 font-mono text-xs leading-5 outline-none focus:border-[#759655]" value={rawImport} onChange={(event) => { setRawImport(event.target.value); setPreview(null); }} placeholder={"H-566\t-\tDomek Rybaka\t2024-07-15\t2024-07-19\tOpłacone\tBooking\t2400\t2024-03-01\t4\t1\t360\t2040\tZakończona"}/></div><div className="rounded-2xl bg-[#e8f0dd] p-5"><span className="grid size-10 place-items-center rounded-xl bg-white text-[#477346]"><Icon className="size-5" name="refresh"/></span><p className="mt-4 text-sm font-black">Bezpieczny import</p><p className="mt-2 text-xs leading-5 text-[#60725e]">Najpierw zobaczysz błędy i wykryte rekordy. Dopiero drugi krok zapisze lub zaktualizuje rezerwacje o tych samych numerach.</p><Button className="mt-5 w-full" disabled={busy || !rawImport.trim()} onClick={() => void previewImport()}>1. Sprawdź dane</Button><Button className="mt-2 w-full" disabled={busy || !preview?.rows.length} variant="secondary" onClick={() => void commitImport()}>2. Scal {preview?.rows.length ?? 0} rekordów</Button></div></div>{preview ? <PreviewTable data={data} preview={preview}/> : null}</Card>
