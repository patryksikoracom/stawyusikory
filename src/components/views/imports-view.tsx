...
export function ImportsView() {
  const { data, replaceWithImportedBookings, updateConnection, exportSnapshot } = useAppStore();
  ...
  async function commitImport() {
    ...
    setBusy(true);
    const response = await fetch("/api/imports/mobile-calendar/commit", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ rows: preview.rows }) });
    setBusy(false);
    if (!response.ok) { setMessage("Walidacja importu nie powiodła się."); return; }
    replaceWithImportedBookings(preview.rows);
    setMessage(`Scalono ${preview.rows.length} rekordów. Istniejące rezerwacje, zadania, płatności i media zostały zachowane.`);
    setRawImport(""); setPreview(null);
  }
  ...
  return <div className="grid gap-5">
    ...
    <div className="grid gap-5 xl:grid-cols-[1fr_360px]">
      <Card className="overflow-hidden"><div className="flex items-start justify-between border-b border-[#e2dbce] p-5 sm:p-6"><div><p className="text-[10px] font-black uppercase tracking-[.16em] text-[#7d8b4d]">Kanały sprzedaży</p><h2 className="font-display text-2xl font-semibold">Kalendarze Booking i Airbnb</h2></div><Badge tone={connected ? "good" : "warn"}>{connected ? "Częściowo aktywne" : "Do konfiguracji"}</Badge></div><div className="grid gap-4 p-4 sm:grid-cols-2">{data.sourceConnections.map((source) => <article className="rounded-2xl border border-[#ded7ca] bg-white p-5" key={source.id}><div className="flex items-start justify-between gap-3"><div className="flex items-center gap-3"><span className={`grid size-11 place-items-center rounded-2xl ${source.platform === "Booking" ? "bg-[#dbeaec] text-[#246675]" : "bg-[#f9dfd8] text-[#b04c37]"}`}><span className="font-display text-xl font-semibold">{source.platform[0]}</span></span><div><p className="font-display text-xl font-semibold">{source.platform}</p><p className="text-xs text-[#6c7872]">{source.connectionType} · {source.unitId ? unitName(data.units, source.unitId) : "brak domku"}</p></div></div><Badge tone={source.status === "Aktywne" ? "good" : source.status === "Błąd" ? "bad" : "warn"}>{source.status}</Badge></div><p className="mt-4 text-sm font-bold leading-6 text-[#43594f]">{source.nextStep}</p>{source.lastSyncAt ? <p className="mt-2 text-xs text-[#748078]">Ostatni odczyt: {new Date(source.lastSyncAt).toLocaleString("pl-PL")}</p> : null}{source.lastError ? <p className="mt-2 text-xs font-bold text-[#9b4029]">{source.lastError}</p> : null}<Button className="mt-4 w-full" variant="secondary" onClick={() => setConfiguring(source)}>Skonfiguruj {source.platform}</Button></article>)}</div></Card>
      <aside className="grid content-start gap-5"><Card className="p-5"><p className="text-[10px] font-black uppercase tracking-[.16em] text-[#7d8b4d]">Eksport Stawy OS</p><h2 className="font-display text-xl font-semibold">Linki dostępności</h2><p className="mt-2 text-xs leading-5 text-[#68756f]">Po wdrożeniu Supabase wygeneruj prywatny token dla każdego domku. W trybie lokalnym możesz sprawdzić format demonstracyjny.</p><div className="mt-4 grid gap-2">{data.units.map((unit) => <a className="flex items-center justify-between rounded-xl border border-[#d8d0c2] bg-white p-3 text-xs font-black" href={`/api/calendar/feeds/demo-${unit.id}`} key={unit.id}>{unit.name}<Icon className="size-4" name="download"/></a>)}</div></Card><Card className="p-5"><p className="text-[10px] font-black uppercase tracking-[.16em] text-[#7d8b4d]">Kopia bezpieczeństwa</p><h2 className="font-display text-xl font-semibold">Eksport pełnych danych</h2><p className="mt-2 text-xs leading-5 text-[#68756f]">Zapisz prywatny plik JSON przed dużym importem lub zmianą konfiguracji.</p><Button className="mt-4 w-full" variant="secondary" onClick={exportSnapshot}><Icon className="size-4" name="download"/>Pobierz backup</Button></Card></aside>
    </div>

    <Card className="overflow-hidden border-[#cedbb9] bg-[#fbfcf5]"><div className="grid gap-5 p-5 lg:grid-cols-[1fr_320px] sm:p-6"><div><p className="text-[10px] font-black uppercase tracking-[.16em] text-[#66813d]">Import jednorazowy</p><h2 className="mt-1 font-display text-2xl font-semibold">Mobile-Calendar: podgląd i scalanie</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[#5d6b61]">Kolumny: numer, gość, domek, przyjazd, wyjazd, płatność; opcjonalnie kanał i cena. Import nigdy nie usuwa istniejących danych.</p><textarea aria-label="Dane z Mobile-Calendar" className="mt-4 min-h-36 w-full rounded-2xl border border-[#cfd8c2] bg-white px-4 py-3 font-mono text-xs leading-5 outline-none focus:border-[#759655]" value={rawImport} onChange={(event) => { setRawImport(event.target.value); setPreview(null); }} placeholder={"566\tGość\tDomek Rybaka\t2026-07-15\t2026-07-19\tBrak wpłaty\tBooking\t2400"}/></div><div className="rounded-2xl bg-[#e8f0dd] p-5"><span className="grid size-10 place-items-center rounded-xl bg-white text-[#477346]"><Icon className="size-5" name="refresh"/></span><p className="mt-4 text-sm font-black">Bezpieczny import</p><p className="mt-2 text-xs leading-5 text-[#60725e]">Najpierw zobaczysz błędy i wykryte rekordy. Dopiero drugi krok zapisze lub zaktualizuje rezerwacje o tych samych numerach.</p><Button className="mt-5 w-full" disabled={busy || !rawImport.trim()} onClick={() => void previewImport()}>1. Sprawdź dane</Button><Button className="mt-2 w-full" disabled={busy || !preview?.rows.length} variant="secondary" onClick={() => void commitImport()}>2. Scal {preview?.rows.length ?? 0} rekordów</Button></div></div>{preview ? <PreviewTable data={data} preview={preview}/> : null}</Card>
...
function PreviewTable({ data, preview }: { data: ReturnType<typeof useAppStore>["data"]; preview: ImportPreview }) { return <div className="border-t border-[#ded7ca] p-4"><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="text-[10px] font-black uppercase tracking-[.13em] text-[#78837d]"><tr><th className="p-3">Rezerwacja</th><th>Termin</th><th>Domek</th><th>Stan</th><th>Braki</th></tr></thead><tbody>{preview.rows.map((booking:Booking)=><tr className="border-t" key={booking.id}><td className="p-3"><p className="font-black">{booking.guestLabel}</p><p className="text-xs">{booking.id}</p></td><td>{booking.checkIn}–{booking.checkOut}<p className="text-xs">{nightsBetween(booking.checkIn,booking.checkOut)} nocy</p></td><td>{unitName(data.units,booking.unitId)}</td><td><Badge tone={data.bookings.some((item)=>item.id===booking.id)?"warn":"good"}>{data.bookings.some((item)=>item.id===booking.id)?"Aktualizacja":"Nowy"}</Badge></td><td className="text-xs">goście · kontakt · źródło</td></tr>)}</tbody></table></div>{preview.errors.length?<div className="mt-3 rounded-xl bg-[#f9dfd7] p-3 text-xs font-bold text-[#963c27]">Pominięte wiersze: {preview.errors.map((item)=>`${item.line}: ${item.message}`).join(" · ")}</div>:null}</div> }
